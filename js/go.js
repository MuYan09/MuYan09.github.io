const safeGoFun = {
            ...
            for (let i = 0; i < links.length; i++) {
                ...
                if (
                    ...
                    if (!(await safeGoFun.NzcheckLocalSite(eleHref)) && !eleIsDownLoad) {
                        ...
                        const encodedHref = btoa(encodeURIComponent(eleHref));
                        ele.setAttribute(
                            "href",
                            "/go.html?goUrl=" + encodedHref
                        );
                    } else if (
                        ...
                        ele.setAttribute(
                            "href",
                            "/go.html?goUrl=" + encodedHref + "&type=goDown"
                        );
                    }
                }
            }
    ...
    NzcheckLocalSite: async (url) => {
        try {
            // 白名单地址则不修改href
            const safeUrls = ["localhost:4000", "blog.3ms.run", "3ms.run", "muooy.com", "www.muooy.com"];
            let isOthers = false;
